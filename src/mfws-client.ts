...
export class MFWSClient extends MFWSClientBase {
  ...
  public async AuthenticateUsingCredentials(authentication: Authentication) {
    ...
    const response = await axios.post(
      `${this.url}/REST/server/authenticationtokens`,
      authentication
    )
  ...
  public async AuthenticateUsingSingleSignOn(vaultGUID: string) {
    // Sanity.
    if (vaultGUID === null) return

    const response = await axios.get(
      `${this.url}/WebServiceSSO.aspx?popup=1&vault=${vaultGUID}`
    )
    return response.data
  }
  ...
  public async GetOnlineVaults(): Promise<Vault[]> {
    const response = await axios.get<Vault[]>(
      `${this.url}/REST/server/vaults?online=true`
    )
    return response.data
  }

  public async GetAuthenticationPlugins(
    vaultGUID: string
  ): Promise<PluginInfoConfiguration[]> {
    let requestUrl = `${this.url}/REST/server/authenticationprotocols`
    // Sanity.
    if (vaultGUID !== null) requestUrl = `${requestUrl}?vault=${vaultGUID}`
